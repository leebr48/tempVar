...
# Variables to alter.
numRun = list(range(660,760,1)) #Number of runs to execute for each variable set. Enumerate these!!! 
t = [2500] #temperatures in Kelvin
orients = ['001'] #Surface orientations as a LIST. Choices: '001', '011', '111', and '310'.
Rs = [4] #Spherical bubble radii. MULTIPLY by ao to get radii in Angstroms. Focusing on smaller bubbles (below continuum limit) is probably best. 
l = [] #Ligament thicknesses. MULTIPLY by ao to get thickness in Angstroms. Go down to 20 ao in the end!  
singleLayer = True #Type 'True' to append an extra run to the 'l' list above, creating runs whose ligament is one atom thick. (Note that this thickness is surface-orientation dependent.) Type 'False' to use only the vaules for 'l' given above. 
...
HeLatParMultip = 3 #Factor used to make the HeLatpar larger than LatPar. 
...
runTime = {'short':'6:00:00','med':'36:00:00','long':'48:00:00'} #Lewis run lengths for small, medium, and large bubbles.
