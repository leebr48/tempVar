...
for tempind in range(len(temps)):
    ...
    for radind in range(len(radii)):
        ...
        plt.scatter(radseries,pseries,color=datacol,facecolors='none',marker=symplot,s=markerArea,alpha=opacity)
        plt.scatter(rad,avgP,color=meancol,marker=symplot,s=markerArea,label=label if radind==0 else '')
    mypredy = magic(radrange,temp)  
    contpredy = cont(radrange,temp)
    maxP.append(max(contpredy))
    plt.plot(radrange,mypredy,color=linecol,linestyle='-')
    plt.plot(radrange,contpredy,color=linecol,linestyle='--')
plt.ylabel(ylab)
